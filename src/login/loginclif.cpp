...
// (^~_~^) Gepard Shield Start
		...
		if(command == 0x0825) {
			char macAdress[17];
			char *mac = RFIFOCP(fd, 60);
			size_t uMacLen = strlen(mac);

			if(!(uMacLen > 17 - 1 || uMacLen == 0))
			{
				safestrncpy(macAdress, mac, uMacLen + 1);
				
				if(strcmp(macAdress,"roBrowser") == 0)
					session[fd]->flag.robrowser = 1;
				else
					session[fd]->flag.robrowser = 0;
			}

		} else session[fd]->flag.robrowser = 0;
		
		if (is_gepard_active == true && !session[fd]->flag.robrowser) {
