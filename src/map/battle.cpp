...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	//PK damage rates
	if (battle_config.pk_mode && sd && bl->type == BL_PC && damage && map_getmapflag(bl->m, MF_PVP) && map_getmapflag(bl->m, MF_PVP2) && map_getmapflag(bl->m, MF_PVP3)) {
...
static int battle_skill_damage_skill(struct block_list *src, struct block_list *target, uint16 skill_id) {
	...
	if ((damage->map&1 && (!mapdata->flag[MF_PVP] && !mapdata->flag[MF_PVP2] && !mapdata_flag_gvg2(mapdata) && !mapdata->flag[MF_BATTLEGROUND] && !mapdata->flag[MF_SKILL_DAMAGE] && !mapdata->flag[MF_RESTRICTED])) ||
		(damage->map&2 && mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP2]) ||
		(damage->map&2 && mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP3]) ||
...
int64 battle_calc_return_damage(struct block_list* bl, struct block_list *src, int64 *dmg, int flag, uint16 skill_id, bool status_reflect){
	...
	if (flag & BF_SHORT) {//Bounces back part of the damage.
		...
		if ( (skill_get_inf2(skill_id, INF2_ISTRAP) || !status_reflect) && sd && sd->bonus.short_weapon_damage_return && status_get_class_(src) != CLASS_BOSS ) {
			rdamage += damage * sd->bonus.short_weapon_damage_return / 100;
			rdamage = i64max(rdamage, 1);
		} //else if( status_reflect && sc && sc->count ) {
			else if( status_reflect && sc && sc->count && status_get_class_(src) != CLASS_BOSS ) {
			...
			if ( sc->data[SC_REFLECTSHIELD] && skill_id != WS_CARTTERMINATION && status_get_class_(src) != CLASS_BOSS ) {
				// Don't reflect non-skill attack if has SC_REFLECTSHIELD from Devotion bonus inheritance
				if (!skill_id && battle_config.devotion_rdamage_skill_only && sc->data[SC_REFLECTSHIELD]->val4)
					rdamage = 0;
				else {
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
				...
				if (!skill_id || su->group->skill_id == NPC_REVERBERATION || su->group->skill_id == WM_POEMOFNETHERWORLD) {
					...
					switch (skill_id) {
						...
						case SR_SKYNETBLOW:
							...
							if (!mapdata->flag[MF_PVP] && !mapdata->flag[MF_PVP2] && !mapdata->flag[MF_PVP3] && !mapdata->flag[MF_GVG])
								return 0;
							break;
					}
				}
	...
	switch( t_bl->type ) { //Checks on target master
		case BL_PC: {
			...
			if(mapdata->flag[MF_PVP] && sd->sc.data[SC_ADD_MATK_DAMAGE] && flag&BCT_ENEMY && s_bl->type != BL_MOB )
				return 0;
	...
	{	//Checks on source master
		case BL_PC:
		{
			...
			if( s_bl != t_bl )
			{
				...
				else if( sd->duel_group && !((!battle_config.duel_allow_pvp && mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP2] && mapdata->flag[MF_PVP3]) || (!battle_config.duel_allow_gvg && mapdata_flag_gvg(mapdata))) )
				{
					if( t_bl->type == BL_PC && (sd->duel_group == ((TBL_PC*)t_bl)->duel_group) )
						return (BCT_ENEMY&flag)?1:-1; // Duel targets can ONLY be your enemy, nothing else.
					else
						return 0; // You can't target anything out of your duel
				} //else if( mapdata->flag[MF_PVP] && sd->sc.data[SC_ADD_MATK_DAMAGE] && t_bl->type != BL_MOB )
					else if(mapdata->flag[MF_PVP] && sd->sc.data[SC_ADD_MATK_DAMAGE] && flag&BCT_ENEMY && t_bl->type != BL_MOB ) // fix party windwalk and other
					return 0;
			}
	...
	{ //Check rivalry settings.
		...
		if( flag&(BCT_PARTY|BCT_ENEMY) )
		{
			...
			if( s_party && s_party == status_get_party_id(t_bl) && !(mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP2] && mapdata->flag[MF_PVP3] && mapdata->flag[MF_PVP_NOPARTY]) && !(mapdata_flag_gvg(mapdata) && mapdata->flag[MF_GVG_NOPARTY]) && (!mapdata->flag[MF_BATTLEGROUND] || sbg_id == tbg_id) )
				state |= BCT_PARTY;
			else
				state |= BCT_ENEMY;
		}
		if( flag&(BCT_GUILD|BCT_ENEMY) )
		{
			...
			if( !(mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP2] && mapdata->flag[MF_PVP3] && mapdata->flag[MF_PVP_NOGUILD]) && s_guild && t_guild && (s_guild == t_guild || (!(flag&BCT_SAMEGUILD) && guild_isallied(s_guild, t_guild))) && (!mapdata->flag[MF_BATTLEGROUND] || sbg_id == tbg_id) )
				state |= BCT_GUILD;
			else
				state |= BCT_ENEMY;
		}
...
static const struct _battle_data {
	...
	{ "job_exp_rate2",                      &battle_config.job_exp_rate,                    100,    0,      INT_MAX,        },
...
// rAthena
	...
	{ "vip_base_exp_increase2",              &battle_config.vip_base_exp_increase2,          50,      0,      INT_MAX,        },
	{ "vip_job_exp_increase2",               &battle_config.vip_job_exp_increase2,           50,      0,      INT_MAX,        },
	...
	{ "vip_drop_increase2",                  &battle_config.vip_drop_increase2,              50,      0,      INT_MAX,        },
