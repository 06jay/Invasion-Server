...
//	if( test_autoloot ) {	//Autoloot.
		...
		if (ditem->item_data.nameid == 35380) {
			//uint32 index;
			int64 uid;
			int64 current;
		...
		if (ditem->item_data.nameid == 35390) {
			//uint32 index;
			int64 uid;
			int64 current;
...
void mob_damage(struct mob_data *md, struct block_list *src, int damage)
{
	...
	//if(md->get_bosstype() == BOSSTYPE_MVP){
	if(md->get_bosstype() == BOSSTYPE_MVP && src){
...
int mob_getdroprate(struct block_list* src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier)
{
	...
	if (src) {
		...
		if (src->type == BL_PC || src->type == BL_MER || src->type == BL_HOM ) { // Player specific drop rate adjustments
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	if(!(type&2) && //No exp
		(!map_getmapflag(m, MF_PVP) || !map_getmapflag(m, MF_PVP2) || !map_getmapflag(m, MF_PVP3) || battle_config.pvp_exp) && //Pvp no exp rule [MouseJstr]
			...
			if(flag) {
				if(base_exp || job_exp) {
					if( md->dmglog[i].flag != MDLF_PET || battle_config.pet_attack_exp_to_master ) {
						...
						if(tmpsd[i] && tmpsd[i]->bl.type == BL_PC && tmpsd[i]->status.guild_id){
							int j_ = 0;
							struct guild* g = guild_search(tmpsd[i]->status.guild_id);
							if(g){
								for (int i_ = 0; i_ < MAX_GUILD; i_++) {
...
//#endif
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			if( mvp_sd && md->db->dropitem[i].rate <= battle_config.rare_drop_announce ) {
				...
				if(ditem->item_data.nameid < 4000 || ditem->item_data.nameid > 4501) {
				sprintf (message, msg_txt(NULL,541), mvp_sd->status.name, md->name, it->ename.c_str(), (float)drop_rate/100);
				intif_broadcast(message,strlen(message)+1,BC_DEFAULT);
				} else {
				sprintf (message, msg_txt(NULL,799), mvp_sd->status.name, md->name, it->ename.c_str(), (float)drop_rate/100);
