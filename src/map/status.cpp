...
#define status_change_isDisabledOnMap(type, m) ( status_change_isDisabledOnMap_((type), mapdata_flag_vs2((m)), m->flag[MF_PVP] && m->flag[MF_PVP2] && m->flag[MF_PVP3] != 0, mapdata_flag_gvg2_no_te((m)), m->flag[MF_BATTLEGROUND] != 0, (m->zone << 3) != 0, mapdata_flag_gvg2_te((m))) )
...
void status_change_clear_onChangeMap(struct block_list *bl, struct status_change *sc)
{
	...
	if (sc && sc->count) {
		...
		bool mapIsPVP = mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP2] && mapdata->flag[MF_PVP3] != 0;
