...
ACMD_FUNC(mapmove)
//{
//	int idx = pc_search_inventory(sd, 35374);
//	if (idx >= 0)
	{
...
//	clif_displaymessage(fd, "You need NFT Warp Command.");
//	return -1;
//}
...
ACMD_FUNC(hide)
{
	...
	if (pc_isinvisible(sd)) {
		...
		if( !battle_config.pk_mode && map_getmapflag(sd->bl.m, MF_PVP) && map_getmapflag(sd->bl.m, MF_PVP2) && !map_getmapflag(sd->bl.m, MF_PVP_NOCALCRANK) )
		...
		if( map_getmapflag(sd->bl.m, MF_PVP) && map_getmapflag(sd->bl.m, MF_PVP2) && !map_getmapflag(sd->bl.m, MF_PVP_NOCALCRANK) && sd->pvp_timer != INVALID_TIMER )
...
ACMD_FUNC(pvpoff2)
{
	...
	if (!map_getmapflag(sd->bl.m, MF_PVP2)) {
		clif_displaymessage(fd, msg_txt(sd,160)); // PvP is already Off.
		return -1;
	}

	map_setmapflag(sd->bl.m, MF_PVP2, false);
...
ACMD_FUNC(pvpon2)
{
	...
	map_setmapflag(sd->bl.m, MF_PVP2, true);
...
ACMD_FUNC(pvpoff3)
{
	...
	if (!map_getmapflag(sd->bl.m, MF_PVP3)) {
		clif_displaymessage(fd, msg_txt(sd,160)); // PvP is already Off.
		return -1;
	}

	map_setmapflag(sd->bl.m, MF_PVP3, false);
...
ACMD_FUNC(pvpon3)
{
	...
	map_setmapflag(sd->bl.m, MF_PVP3, true);
...
ACMD_FUNC(go)
{
	//int idx = pc_search_inventory(sd, 35373);
	//if (idx >= 0)
	//{
	int i;
	int town;
...
ACMD_FUNC(reload) {
	...
	if (strstr(command, "itemdb") || strncmp(message, "itemdb", 4) == 0) {
		...
		else if (prev_config.online_users != battle_config.online_users || prev_config.online_users_nocount != battle_config.online_users_nocount) {
			send_users_tochar();
		}
...
ACMD_FUNC(mapinfo) {
	...
	if (map_getmapflag(m_id, MF_PVP) && map_getmapflag(m_id, MF_PVP2))
...
ACMD_FUNC(autotrade) {
	...
	int idx = pc_search_inventory(sd, 501);
	if (idx >= 0)
	{
	nullpo_retr(-1, sd);
	...
	clif_displaymessage(fd, "You need NFT Autotrade Command.");
	return -1;
}
...
//	clif_displaymessage(fd, "You need NFT Autoloot Command.");
//	return -1;
//}
		...
		if(sd->sc.data[SC_ITEMBOOST]){
			for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
				...
				if (item_data->slots)
					...
					else if(item_data->nameid < 4000 || item_data->nameid > 4501)
					sprintf(atcmd_output2, " - %s  %02.02f%%", item_data->ename.c_str(),  (float)droprate / 100);
					else{
					if(sd->sc.data[SC_ADD_ATK_DAMAGE]){
                   sprintf(atcmd_output2, " - %s 0.10%%", item_data->ename.c_str());
               } else {
                   sprintf(atcmd_output2, " - %s 0.05%%", item_data->ename.c_str());
               }
           }
			...
			clif_displaymessage(fd, msg_txt(sd,798)); // Monster Drops has been disabled on Standard Accounts.
		if (j == 0)
...
ACMD_FUNC(rates)
{
	...
	if (sd->sc.data[SC_ADD_MATK_DAMAGE]) {
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1298), // Experience rates: Base %.2fx / Job %.2fx
		(battle_config.base_exp_rate + (sd->sc.data[SC_ADD_MATK_DAMAGE] ? (battle_config.vip_base_exp_increase2 * battle_config.base_exp_rate) / 100 : 0)) / 100.,
		(battle_config.job_exp_rate + (sd->sc.data[SC_ADD_MATK_DAMAGE] ? (battle_config.vip_job_exp_increase2 * battle_config.job_exp_rate) / 100 : 0)) / 100.);
	} else {
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1298), // Experience rates: Base %.2fx / Job %.2fx
		(battle_config.base_exp_rate + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_base_exp_increase * battle_config.base_exp_rate) / 100 : 0)) / 100.,
		(battle_config.job_exp_rate + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_job_exp_increase * battle_config.job_exp_rate) / 100 : 0)) / 100.);
	}
	...
	if(sd->sc.data[SC_ADD_ATK_DAMAGE]){
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1299), // Normal Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
			(battle_config.item_rate_common + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_common) / 100 : 0)) / 100.,
			(battle_config.item_rate_heal + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_heal) / 100 : 0)) / 100.,
			(battle_config.item_rate_use + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_use) / 100 : 0)) / 100.,
			(battle_config.item_rate_equip + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_equip) / 100 : 0)) / 100.,
			(battle_config.item_rate_card + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_card) / 100 : 0)) / 100.);
		clif_displaymessage(fd, buf);
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1300), // Boss Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
			(battle_config.item_rate_common_boss + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_common_boss) / 100 : 0)) / 100.,
			(battle_config.item_rate_heal_boss + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_heal_boss) / 100 : 0)) / 100.,
			(battle_config.item_rate_use_boss + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_use_boss) / 100 : 0)) / 100.,
			(battle_config.item_rate_equip_boss + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_equip_boss) / 100 : 0)) / 100.,
			(battle_config.item_rate_card_boss + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_card_boss) / 100 : 0)) / 100.);
		clif_displaymessage(fd, buf);
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1024), // MVP Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
			(battle_config.item_rate_common_mvp + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_common_mvp) / 100 : 0)) / 100.,
			(battle_config.item_rate_heal_mvp + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_heal_mvp) / 100 : 0)) / 100.,
			(battle_config.item_rate_use_mvp + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_use_mvp) / 100 : 0)) / 100.,
			(battle_config.item_rate_equip_mvp + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_equip_mvp) / 100 : 0)) / 100.,
			(battle_config.item_rate_card_mvp + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_card_mvp) / 100 : 0)) / 100.);
		clif_displaymessage(fd, buf);
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1301), // Other Drop Rates: MvP %.2fx / Card-Based %.2fx / Treasure %.2fx
			(battle_config.item_rate_mvp + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_mvp) / 100 : 0)) / 100.,
			(battle_config.item_rate_adddrop + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_adddrop) / 100 : 0)) / 100.,
			(battle_config.item_rate_treasure + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase * battle_config.item_rate_treasure) / 100 : 0)) / 100.);
		clif_displaymessage(fd, buf);
	} else 
	if(sd->sc.data[SC_ITEMBOOST]){
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1299), // Normal Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
			(battle_config.item_rate_common + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_common) / 100 : 0)) / 100.,
			(battle_config.item_rate_heal + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_heal) / 100 : 0)) / 100.,
			(battle_config.item_rate_use + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_use) / 100 : 0)) / 100.,
			(battle_config.item_rate_equip + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_equip) / 100 : 0)) / 100.,
			(battle_config.item_rate_card + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_card) / 100 : 0)) / 100.);
		clif_displaymessage(fd, buf);
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1300), // Boss Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
			(battle_config.item_rate_common_boss + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_common_boss) / 100 : 0)) / 100.,
			(battle_config.item_rate_heal_boss + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_heal_boss) / 100 : 0)) / 100.,
			(battle_config.item_rate_use_boss + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_use_boss) / 100 : 0)) / 100.,
			(battle_config.item_rate_equip_boss + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_equip_boss) / 100 : 0)) / 100.,
			(battle_config.item_rate_card_boss + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_card_boss) / 100 : 0)) / 100.);
		clif_displaymessage(fd, buf);
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1024), // MVP Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
			(battle_config.item_rate_common_mvp + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_common_mvp) / 100 : 0)) / 100.,
			(battle_config.item_rate_heal_mvp + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_heal_mvp) / 100 : 0)) / 100.,
			(battle_config.item_rate_use_mvp + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_use_mvp) / 100 : 0)) / 100.,
			(battle_config.item_rate_equip_mvp + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_equip_mvp) / 100 : 0)) / 100.,
			(battle_config.item_rate_card_mvp + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_card_mvp) / 100 : 0)) / 100.);
		clif_displaymessage(fd, buf);
		snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1301), // Other Drop Rates: MvP %.2fx / Card-Based %.2fx / Treasure %.2fx
			(battle_config.item_rate_mvp + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_mvp) / 100 : 0)) / 100.,
			(battle_config.item_rate_adddrop + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_adddrop) / 100 : 0)) / 100.,
			(battle_config.item_rate_treasure + (sd->sc.data[SC_ITEMBOOST] ? (battle_config.vip_drop_increase2 * battle_config.item_rate_treasure) / 100 : 0)) / 100.);
		clif_displaymessage(fd, buf);
	} else 
		clif_displaymessage(fd, msg_txt(sd,797)); // Drop Rates has been disabled on Standard Accounts.
	return 0;
}
...
ACMD_FUNC( stylist ){
	...
	clif_displaymessage( fd, msg_txt( sd, 1516 ) ); // This command requires packet version 2015-11-04 or newer.
	return -1;
#else

	if( sd->state.stylist_open ){
		clif_displaymessage( fd, msg_txt( sd, 1517 ) ); // You have already opened the stylist UI.
		return -1;
	}
