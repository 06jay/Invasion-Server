...
// Auto-Attack Players teleports if 'battle_config.autoatk_autotp_range' cells near from npc warp portal [RagservPH]
int npc_autoattack_check(struct map_session_data* sd)
{
	int i, x = sd->bl.x, y = sd->bl.y;
	...
	for( i = 0; i < mapdata->npc_num_area; i++ )
	{
		...
		if(mapdata->npc[i]->subtype == NPCTYPE_WARP &&
			x >= mapdata->npc[i]->bl.x-battle_config.autoatk_autotele_range && x <= mapdata->npc[i]->bl.x+battle_config.autoatk_autotele_range &&
			y >= mapdata->npc[i]->bl.y-battle_config.autoatk_autotele_range && y <= mapdata->npc[i]->bl.y+battle_config.autoatk_autotele_range
		) {
