...
bool pc_authok(struct map_session_data *sd, uint32 login_id2, time_t expiration_time, int group_id, struct mmo_charstatus *st, bool changing_mapservers)
{
	...
	sd->canteleport_tick = tick; // disable tele
...
void pc_reg_received(struct map_session_data *sd)
{
	...
	if (sd->state.connect_new == 0 && sd->fd) { //Character already loaded map! Gotta trigger LoadEndAck manually.
		...
		if( pc_isinvisible( sd ) ){
			...
			if( map_getmapflag( sd->bl.m, MF_PVP ) && map_getmapflag( sd->bl.m, MF_PVP2 ) && map_getmapflag( sd->bl.m, MF_PVP3 ) && !map_getmapflag( sd->bl.m, MF_PVP_NOCALCRANK ) && sd->pvp_timer != INVALID_TIMER ){
...
bool pc_takeitem(struct map_session_data *sd,struct flooritem_data *fitem)
{
	...
	if (fitem->item.nameid == 35380) {
		//uint32 index;
		int64 uid;
		int64 current;
		uid = reference_uid(add_str("$BOSSS"), 0);
		current = mapreg_readreg(uid);
		setd_sub_num(NULL, sd, "$BOSSS", 0, current + 1, NULL);
	}

	if (fitem->item.nameid == 35390) {
		//uint32 index;
		int64 uid;
		int64 current;
		uid = reference_uid(add_str("$WOES"), 0);
		current = mapreg_readreg(uid);
		setd_sub_num(NULL, sd, "$WOES", 0, current + 1, NULL);
	}
...
bool pc_isUseitem(struct map_session_data *sd,int n)
{
	struct item_data *item;
	t_itemid nameid;
	t_tick tick = gettick(); // disable tele
	bool wing = false; // disable tele
	...
	switch( nameid ) {
		...
		//ShowError(" diff tick %d wing %d \n",DIFF_TICK(sd->canteleport_tick,tick),wing);
			if(DIFF_TICK(sd->canteleport_tick,tick)>0){ // disable tele
				//if(!wing)
					clif_displaymessage(sd->fd,"There is a 10 seconds delay before being able to teleport");
				return false;
			}
...
bool pc_steal_item(struct map_session_data *sd,struct block_list *bl, uint16 skill_lv)
{
	...
	if(!sd || !bl || bl->type!=BL_MOB || !sd->sc.data[SC_ITEMBOOST] )
		return false;
...
void pc_gainexp(struct map_session_data *sd, struct block_list *src, t_exp base_exp, t_exp job_exp, uint8 exp_flag)
{
	...
	if (!(exp_flag&2)) {
		...
		if (!battle_config.pvp_exp && map_getmapflag(sd->bl.m, MF_PVP) && map_getmapflag(sd->bl.m, MF_PVP2) && map_getmapflag(sd->bl.m, MF_PVP3))  // [MouseJstr]
			return; // no exp on pvp maps
	
		if (sd->status.guild_id>0)
		//base_exp -= guild_payexp(sd,base_exp);
		if (sd->status.guild_id>0 && sd->status.base_level >= 56)
      base_exp -= guild_payexp(sd,base_exp);
...
void pc_damage(struct map_session_data *sd,struct block_list *src,unsigned int hp, unsigned int sp, unsigned int ap)
{
	...
	if(battle_config.prevent_logout_trigger&PLT_DAMAGE)
		sd->canlog_tick = gettick();
	//ShowError("src %d sd %d \n",src->id,sd->bl.id);
	if (src && src->type == BL_PC && src->id != sd->bl.id) {
	//if(src && src->type == BL_PC){
		t_tick hittick = gettick();
		int teleportdelaycd = 10000; //This is the delay in milliseconds you can set what ever delay you want
		sd->canteleport_tick = hittick+teleportdelaycd;
		struct map_session_data *tsd = (map_session_data *)src;
    	tsd->canteleport_tick = hittick+teleportdelaycd;
	//ShowError("damaged PC %d \n",sd->canteleport_tick);
	} else if(src && src->type == BL_MOB){
...
int pc_dead(struct map_session_data *sd,struct block_list *src)
{
	...
	if(battle_config.bone_drop==2
		|| (battle_config.bone_drop==1 && mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP2] && mapdata->flag[MF_PVP3]))
	{
	...
	if( !battle_config.pk_mode && mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP2] && mapdata->flag[MF_PVP3] && !mapdata->flag[MF_PVP_NOCALCRANK] ) {
		sd->pvp_point -= 5;
		sd->pvp_lost++;
		if( src && src->type == BL_PC ) {
			struct map_session_data *ssd = (struct map_session_data *)src;
