...
void clif_map_property(struct block_list *bl, enum map_property property, enum send_target t)
{
	...
	WBUFL(buf,4) = ((mapdata->flag[MF_PVP]?1:0 || mapdata->flag[MF_PVP2]?1:0 || mapdata->flag[MF_PVP3]?1:0 || (bl->type == BL_PC && ((TBL_PC *)bl)->duel_group > 0))<<0)| // PARTY - Show attack cursor on non-party members (PvP)
		...
		((mapdata->flag[MF_NOSUNMOONSTARMIRACLE]?0:1)<<10)|
		((mapdata->flag[MF_PVP2]?1:0)<<11)| // COUNT_PK - Show the PvP counter
		((mapdata->flag[MF_PVP3]?1:0)<<12); // COUNT_PK - Show the PvP counter; // SUNMOONSTAR_MIRACLE - Blocks Star Gladiator's Miracle from activating
...
inline void clif_pk_mode_message(struct map_session_data * sd)
{
	if (battle_config.pk_mode && battle_config.pk_mode_mes &&
		sd && map_getmapflag(sd->bl.m, MF_PVP) && map_getmapflag(sd->bl.m, MF_PVP2) && map_getmapflag(sd->bl.m, MF_PVP3)) {
...
// (^~_~^) Gepard Shield Start
	...
	if (is_gepard_active && !session[fd]->flag.robrowser)
	{
...
void clif_parse_LoadEndAck(int fd,struct map_session_data *sd)
{
	...
	if(!pc_isinvisible(sd) && mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP2] && mapdata->flag[MF_PVP3]) {
...
// (^~_~^) Gepard Shield Start
	...
	if (!session[fd]->flag.robrowser && is_gepard_active == true && sd != NULL && clif_gepard_process_packet(sd) == true)
	{
		return 0;
	}
...
// [AndRO joystickmove]
void clif_parse_andro_joystickmove(int fd, struct map_session_data *sd)
{
	char dir = RFIFOB(fd, 2);
	const short dirx[] = { 0, -1, -1, -1, 0, 1, 1, 1 }; ///lookup to know where will move to x according dir
	const short diry[] = { 1, 1, 0, -1, -1, -1, 0, 1 }; ///lookup to know where will move to y according dir
