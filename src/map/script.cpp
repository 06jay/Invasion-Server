...
BUILDIN_FUNC(pvpon2)
{
	...
	if (!map_getmapflag(m, MF_PVP2))
		map_setmapflag(m, MF_PVP2, true);
...
BUILDIN_FUNC(pvpon3)
{
	...
	if (!map_getmapflag(m, MF_PVP3))
		map_setmapflag(m, MF_PVP3, true);

	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(pvpoff2)
{
	int16 m;
	const char *str = script_getstr(st,2);
	...
	if (map_getmapflag(m, MF_PVP2))
		map_setmapflag(m, MF_PVP2, false);

	return SCRIPT_CMD_SUCCESS;
}
BUILDIN_FUNC(pvpoff3)
{
	int16 m;
	const char *str = script_getstr(st,2);
	...
	if (map_getmapflag(m, MF_PVP3))
		map_setmapflag(m, MF_PVP3, false);
...
BUILDIN_FUNC(setticketuniqueid)
{
	...
	if( sd->itemid == 0 ){
		ShowError( "setticketuniqueid: Called outside of an item script without item id.\n" );
		return SCRIPT_CMD_FAILURE;
	}

	if( !sd->inventory_data[sd->itemindex]->flag.guid ){
		ShowError( "setticketuniqueid: Called from item %u, which has not an unique id.\n", sd->itemid );
		return SCRIPT_CMD_FAILURE;
	}
...
// (^~_~^) Gepard Shield End
	...
	BUILDIN_DEF(pvpon2,"s"),
	BUILDIN_DEF(pvpoff2,"s"),
	BUILDIN_DEF(gvgon,"s"),
