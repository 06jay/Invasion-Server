...
uint64 PetDatabase::parseBodyNode( const YAML::Node &node ){
	...
	if( this->nodeExists( node, "NoLeaveMaster" ) ){
		uint16 nlm;

		if( !this->asUInt16( node, "NoLeaveMaster", nlm ) ){
			return 0;
		}

		if( nlm > 1 ){
			this->invalidWarning( node["NoLeaveMaster"], "NoLeaveMaster %hu exceeds maximum of 1. Capping...\n", nlm );
			nlm = 1;
		}
...
void pet_set_intimate(struct pet_data *pd, int value)
{
	...
	if(pet_db_ptr->nlm && pd->pet.intimate < PET_INTIMATE_NONE + 50){
		pd->pet.intimate = PET_INTIMATE_NONE + 50;
		pd->pet.hungry = PET_HUNGRY_VERY_HUNGRY;
	}
