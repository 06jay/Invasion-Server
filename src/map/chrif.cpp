...
TIMER_FUNC(send_usercount_tochar){
	chrif_check(-1);
	int users = 0, users2 = 0;
	struct map_session_data* sd;
	struct s_mapiterator* iter;

	users = map_usercount();
	users2 = users + battle_config.online_users;

	iter = mapit_getallusers();
	for (sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC*)mapit_next(iter)) {
		if (battle_config.online_users_nocount&0x1 && sd->state.autotrade)
			--users2;
	}
	mapit_free(iter);
...
int send_users_tochar(void) {
	...
	WFIFOHEAD(char_fd, 8 + 8 * users);
	WFIFOW(char_fd,0) = 0x2aff;

	iter = mapit_getallusers();

	for( sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC*)mapit_next(iter) ) {
		WFIFOL(char_fd,8+8*i) = sd->status.account_id;
		WFIFOL(char_fd,8+8*i+4) = sd->status.char_id;
		i++;
		if (battle_config.online_users_nocount&0x1 && sd->state.autotrade)
			--users2;
	}
	...
	WFIFOW(char_fd,2) = 8 + 8*users;
	WFIFOW(char_fd,4) = users; // Actual users
	WFIFOW(char_fd,6) = users2; // Modified user number
	WFIFOSET(char_fd, 8+8*users);
