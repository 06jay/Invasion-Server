...
void map_flags_init(void){
	for (int i = 0; i < map_num; i++) {
		...
		if( battle_config.pk_mode && !mapdata_flag_vs2(mapdata) )
			...
			//mapdata->flag[MF_PVP2] = true; // make all maps pvp for pk_mode [Valaris]
	}
}
	...
	switch(mapflag) {
		...
		case MF_GVG_TE:
			...
			if (!status) {
				...
				if (mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP2] && mapdata->flag[MF_PVP3]) {
					mapdata->flag[MF_PVP] = false;
					mapdata->flag[MF_PVP2] = false;
					mapdata->flag[MF_PVP3] = false;
					if (!battle_config.pk_mode)
		...
		case MF_GVG_TE_CASTLE:
			...
			break;
		case MF_GVG_DUNGEON:
			if (status && mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP2] && mapdata->flag[MF_PVP3]) {
				mapdata->flag[MF_PVP] = false;
				mapdata->flag[MF_PVP2] = false;
				mapdata->flag[MF_PVP3] = false;
				if (!battle_config.pk_mode)
